# PowerShell script to add chatbot to all HTML files
# This script will add the chatbot component to all HTML files in the current directory
...
$htmlFiles = Get-ChildItem -Path "." -Filter "*.html" | Where-Object { $_.Name -notin @("chatbot.html") }
...
foreach ($file in $htmlFiles) {
    ...
    if ($content -match "chatbot-container") {
        Write-Host "  - Chatbot already exists, skipping..."
        continue
    }
    
    # Find the closing </body> tag and insert chatbot before it
    if ($content -match "</body>") {
        $newContent = $content -replace "</body>", "$chatbotComponent`n</body>"
        
        # Write the updated content back to the file
        Set-Content -Path $file.FullName -Value $newContent -Encoding UTF8
        
        Write-Host "  - Chatbot added successfully!"
    } else {
        Write-Host "  - Warning: No </body> tag found, skipping..."
    }
}

Write-Host "`nChatbot addition process completed!"
